...
| Schedule reconciliation | Upsert on every runtime start; equal fingerprint keeps timing state, changed fingerprint resets nextFireAt, clears the lease and bumps the fence so in-flight claims of the old definition cannot complete (last writer wins across rolling deploys; refined 2026-09-10 by the Postgres contract tests); a schedule removed from code keeps its row and stops firing, because `dueSchedules` is filtered by the ids the node holds definitions for (the schedule analogue of claim-by-bound-kinds); never auto-deleted | 2026-09-04 |
